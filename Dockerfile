...
RUN apt-get -y update && apt-get install -y openssh-server wget curl python git git-core make build-essential g++ jq maven screen curl make openjdk-7-jdk unzip
RUN sed -i 's|session    required     pam_loginuid.so|session    optional     pam_loginuid.so|g' /etc/pam.d/sshd
RUN mkdir -p /var/run/sshd

# Install phantomjs
RUN apt-get -y update && apt-get install -y phantomjs
